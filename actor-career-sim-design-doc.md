...
- **All-Time Blockbuster requires scale, not just ratio**: worldwide gross must rank among the biggest in the universe's recent history — **top-10 of the last five years** — so the bar rises as the industry's economy grows across a 50-year career, the label stays rare by construction (roughly two films a year at most), and a tiny film tripling its cost is not an "all-timer". *(Confirmed 2026-09-12 after considering, and rejecting, a fixed $100M floor: a fixed floor becomes routine once tentpoles gross $500M+.)* Cold start (Phase 4 implementation, 2026-09-12): the window counts films whose runs *ended* in the last five years; while fewer than ten such films exist, the film must out-gross every one of them; with none at all the gate is zero. The 52-week prehistory means the window is never empty once a career begins.
...
5. Perception, typecasting, rivalries, relationships, news.
   *Phase 5 decisions (2026-09-13):* perception is shown as a word band ("Buzz"), never a number, and contracts/offers/callbacks read the perceived value; the typecasting identity is shown as a badge on the player's profile; media events with decisions run at about three a year. *Additions agreed for Phase 5:* momentum decay + a floor for established names; a Rivals panel with a scoreboard; genre-era news lines off the drifting trends; minimal sequels (a Hit+ can spawn one ~2 years later, the studio remembers the original cast, held sequel options are exercised); "offers you passed on" news; Connections/Charisma/Professionalism become growable; on-set events driven by stats and relationships, some with decisions; Critical Rep decay with diminishing returns near 100 and a Reputation that can fall; performance-side gains scaled by role; studio favouritism (a studio you made money for calls earlier and opens higher; one you burned goes quiet); age-appropriate roles as story. Agent memory was considered and left out.
5.5. Entity profile pages (people + movies), pulled forward from Phase 7 because the per-entity history already exists after Phases 2–4 and Phase 5's rivals/relationships make the pages worth reading. Plain versions; Phase 7 polishes. Awards sections show "none yet" until Phase 6.
